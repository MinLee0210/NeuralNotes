...
from .miditoolkit.midi.containers import Note, TempoChange, Marker, Instrument

device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

def numpy_to_tensor(arr, use_gpu=True, device='cuda'):
  if use_gpu:
        ...
        for item in groups[i][1:-1]:
            ...
            if item.name == 'Note':
                ...
                events.append(Event(
                    name='Note Duration',
                    time=item.start,
                    value=DEFAULT_DURATION_BINS[index],
                ...
                if tempo in DEFAULT_TEMPO_INTERVALS[0]:
                    ...
                    # tempo_style = Event('Tempo Class', item.start, 'slow', None)
                    tempo_value = Event('Tempo', item.start, 0, None)
                elif tempo > DEFAULT_TEMPO_INTERVALS[2].stop:
                    # tempo_style = Event('Tempo Class', item.start, 'fast', None)
                    tempo_value = Event('Tempo', item.start, 59, None)
...
def get_remi_features(events):
  bar_event = []
  feature_ext = []

  def get_definition(event):
    name = event.name
    value = event.value
  ...
  for idx in range(len(events)):
    ...
    event = get_definition(events[idx])
    feature_ext.append(event)

  result = (bar_event, feature_ext)
  return result

def group_pieces(content, resource, n_pieces=2):
    pieces = []
    pieces.append(content)
    files = [pkl_file for pkl_file in os.listdir(resource) if '.pkl' in pkl_file]
    for idx in range(n_pieces): 
        rand_idx = np.random.randint(0, len(files))
        while files[rand_idx] in pieces: 
            rand_idx = np.random.randint(0, len(files))
        pieces.append(files[rand_idx])
...
def write_midi(words, word2event, output_path, prompt_path=None):
    ...
    for note in temp_notes:
        if note == 'Bar':
            ...
            # duration (end time)
            et = st + duration
            notes.append(Note(velocity, pitch, st, et))
    ...
    if prompt_path:
        ...
        for st, bpm in tempos:
            st += last_time
            temp_tempos.append(TempoChange(bpm, st))
        midi.tempo_changes = temp_tempos
        # write chord into marker
        if len(temp_chords) > 0:
            for c in chords:
                midi.markers.append(
                    Marker(text=c[1], time=c[0]+last_time))
    else:
        midi = MidiFile()
        midi.ticks_per_beat = DEFAULT_RESOLUTION
        # write instrument
        inst = Instrument(0, is_drum=False)
        inst.notes = notes
        midi.instruments.append(inst)
        # write tempo
        tempo_changes = []
        for st, bpm in tempos:
            tempo_changes.append(TempoChange(bpm, st))
        midi.tempo_changes = tempo_changes
        # write chord into marker
        if len(temp_chords) > 0:
            for c in chords:
                midi.markers.append(
                    Marker(text=c[1], time=c[0]))
    # write
...
def midi_to_wav(mid_file, soundfont, endpoint, sample_rate=22050):
    fs = FluidSynth(sound_font=soundfont, sample_rate=sample_rate)
    fs.midi_to_audio(mid_file, f'{endpoint}')

# ==================================================================
#                          WORD CONTROL
# ==================================================================

def control_beat(word, vocab_len): 
    """
        If the word is out of vocab range, the value of word is randomly assigned a value of Beat. 
        Therefore, if there is update in vocabulary, the function should be updated to. 
    """
    if word > vocab_len: 
        word = np.random.randint(1, 17)
