...
from .miditoolkit.midi.containers import Instrument, Marker, TempoChange, Note
...
def remi2midi(events, output_midi_path=None, is_full_event=False, return_first_tempo=False, enforce_tempo=False, enforce_tempo_val=None):
  ...
  for n in temp_notes:
    midi_obj.instruments[0].notes.append(
      Note(int(n.velocity), n.pitch, int(n.start_tick), int(n.start_tick + n.duration))
    )

  if enforce_tempo is False:
    for t in temp_tempos:
      midi_obj.tempo_changes.append(
        TempoChange(t.tempo, int(t.start_tick))
      )
  else:
  ...
  for c in temp_chords:
    midi_obj.markers.append(
      Marker('Chord-{}'.format(c.chord_val), int(c.start_tick))
    )
  for b in range(cur_bar):
    midi_obj.markers.append(
      Marker('Bar-{}'.format(b+1), int(DEFAULT_BAR_RESOL * b))
    )
