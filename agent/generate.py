...
config_path = r'D:\UNI\Thesis\NeuralNotes\agent\config\musemorphose_small_v1.yaml'
config = yaml.load(open(config_path, 'r'), Loader=yaml.FullLoader)

device = 'cuda' if torch.cuda.is_available() else 'cpu'
factory = Factory(version='version_3')

# data_split = 'pickles/test_pieces.pkl'


# ckpt_path = r'D:\UNI\Thesis\NeuralNotes\agent\cktp\mm_small.pt'
# out_dir = 'results'
...
def generate_dataloader(data_dir, pieces, vocab_path):
...
def generate(dset, out_dir, soundfont_dir, n_pieces=2, n_samples_per_piece=3):
  ...
  for p in pieces:
    ...
    orig_filename = orig_out_file
    orig_mid_file = orig_filename + '.mid'
    _, orig_tempo = remi2midi(orig_song, orig_mid_file, return_first_tempo=True, enforce_tempo=False)
    # file_storage.append(orig_mid_file)
    # save metadata of reference song (events & attr classes)
    print (*orig_song, sep='\n', file=open(orig_out_file + '.txt', 'a'))
    # np.save(orig_out_file + '_POLYCLS.npy', p_data['polyph_cls_bar'])
    # np.save(orig_out_file + '_RHYMCLS.npy', p_data['rhymfreq_cls_bar'])


    for k in p_data.keys():
      if not torch.is_tensor(p_data[k]):
        # print('torch.is_tensor == not', p_data[k])
        if type(p_data[k]) == str:
          p_data[k] = np.int32(p_data[k])
    ...
    for samp in range(n_samples_per_piece):
      ...
      out_file = os.path.join(out_dir, 'id{}_bar{}_sample{:02d}_poly{}_rhym{}'.format(
        p, p_bar_id, samp + 1,
        '_plus_{}'.format(p_cls_diff[samp]) if p_cls_diff[samp] >= 0 else '_minus_{}'.format(np.abs(p_cls_diff[samp])), 
        '_plus_{}'.format(r_cls_diff[samp]) if r_cls_diff[samp] >= 0 else '_minus_{}'.format(np.abs(r_cls_diff[samp]))
      ))          
      ...
      mid_filename = out_file + '.mid'
      remi2midi(song, mid_filename, enforce_tempo=True, enforce_tempo_val=orig_tempo)
      file_storage.append(mid_filename)
